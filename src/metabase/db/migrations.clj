...
;;
;; 1. Create three new collections within the root collection: "Migrated dashboards," "Migrated pulses," and "Migrated
;;    questions."
;;
;; 2. The permissions settings for these new collections should be set to No Access for all user groups except
;;    Administrators.
;;
;; 3. Existing Dashboards, Pulses, and Questions from the "Everything Else" area should now be present within these
;;    new collections.
;;
;; 4. Set permissions to the Root Collection to view access for All User group. (This is done last so we don't end up
;;    copying the parent perms to the new 'Migrated' collections.) IMPORTANT: This should only be done for instances
;;    that are being 'migrated' over to 30 -- DO NOT do it for new instances!!!
;;
(defmigration ^{:author "camsaul", :added "0.30.0"} add-migrated-collections
  (let [migrated-anything? (atom false)]
    ...
    (doseq [[model new-collection-name] {Dashboard (trs "Migrated Dashboards")
      ...
      (reset! migrated-anything? true)
      ;; 2. make sure all-users doesn't have any perms for this collection. They shouldn't, because when parent perms
      ;; get copied (from the Root Collection) they shouldn't have any perms yet. But just to be safe make sure they
      ;; don't!
      (perms/revoke-collection-permissions! (perm-group/all-users) new-collection)
      ;; 3. move everything not in this Collection to a new Collection
    ...
    ;; 4. Finally, grant Root Collection read perms to All Users, but only if we migrated anything...
    (when @migrated-anything?
      (perms/grant-collection-read-permissions! (perm-group/all-users) collection/root-collection))))
